...
一个 bot，组建你的 Claude Code 项目管理团队。一人群控所有项目，或将 AI 超能力共享给团队，同一个群里各司其职、互不干扰。
...
主控机器人提供完整的按钮菜单 —— 管理机器人、编辑配置、控制用户权限，全部在 Telegram 中完成，初始安装后无需终端。
...
- **引导式设置** —— 一步步带你添加 bot、设置群组、配置项目
- **机器人管理** —— 通过按钮添加、删除、配置项目机器人
- **配置编辑器** —— 所有设置带说明和验证，按钮操作
- **用户管理** —— 管理员 + 按 bot 成员权限，按钮操作
- **项目看板** —— 置顶消息：git 状态、context 用量、费用、命令参考

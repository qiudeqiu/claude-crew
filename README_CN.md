...
2. 在群里发 `@master setup` —— 交互式向导引导你完成：
   - 设置此群组为共享控制群组
   - 添加第一个项目机器人（token → 项目名 → 路径）
   - 一键重启上线
3. 用 `@master bots` 添加更多机器人，`@master config` 编辑设置，`@master users` 管理权限
...
> **替代方案：** 跳过此步。启动 daemon 后，在 Telegram 中通过 `@master bots` → 添加机器人 来交互式添加项目机器人。

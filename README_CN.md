...
将 Claude Code 的完整能力释放到个人多项目远程管理和团队多项目远程协作 — 在一个 IM 群里。

一个人管理所有项目，或带上你的团队一起协作 — 每个项目一个专属 bot，@提及即可调度，所有动态汇聚在同一条时间线上。**主控机器人**是你的控制中心：添加项目机器人、修改配置、管理团队权限、监控状态，全部通过手机上的按钮菜单完成。每个**项目机器人**对接一个代码仓库，@提及即可执行 Claude Code 任务。一个群，所有项目，各司其职。
...
所有请求、进度、结果、团队讨论汇聚在同一条时间线。Bot 完成任务的瞬间，整个团队都看到 — 不用截图、不用"你看下我的终端"。成员在 bot 动态之间直接沟通，实时跟进结果。每条 bot 回复自动带 `#项目名` 标签，点击即可筛选该项目的完整历史。
...
| **多签审批** | 需要几人确认 | `approvalRequired: 2` — 按钮显示"允许 (1/2)"，凑够才执行 |
| **审批委托** | 临时授权 | `/delegate @user 2h` — 授予审批权限，到期自动回收 |
| **写操作投票** | 团队审查代码修改 | 开启后，修改了文件的结果带 [调整] [确认] 按钮，可配投票人和阈值 |

全局或 per-bot 配置，全在手机按钮菜单里完成。敏感项目设 `readOnly`，个人项目设 `allowAll`，团队项目设 `approve` + 多签 — 写操作需要多人确认。
...
- **写操作投票**：修改了文件的结果自动带 [调整] [确认] 按钮 — 可配置投票人和阈值
- **多签审批**：approve 模式支持多人确认 — 按钮显示"允许 (1/2)"，达标才执行
- **审批委托**：`/delegate @user 2h` 授予临时审批权限 — 到期自动回收
